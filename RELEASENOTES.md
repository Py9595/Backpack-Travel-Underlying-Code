...
本文档包含了backpacktravelalliances的各个版本的注释。

关于backpacktravelalliances的编译方法，请参考 [编译](./Builds)。
关于如果运行backpacktravelalliances服务，请参考 [网络架设](http://www.backpacktravelalliances.net/setup.html)。

关于backpacktravelalliances的其它详细介绍，请访问 [www.backpacktravelalliances.net](http://www.backpacktravelalliances.net)。

# 发布

## 版本 0.20.1

backpacktravelalliances的主要功能：

- **数据库基础功能**：支持增删改查等基本数据库功能。

- **事务支持**：可同时对多张表进行多项操作。

- **表同步**：节点加入backpacktravelalliances链后可同步链上存在的数据库表，可同步加密（不加密）表到指定的时间、ledger或交易hash。
