...
namespace ripple {
    ...
    {      
		...
		BackpacktravelAlliancesTx& backpacktravelAlliancesTx = dynamic_cast<BackpacktravelAlliancesTx&>(transactor);
		...
		for (auto& eachTx : vecTxs)
		{
			...
			auto ret = TableStorageHandlePut(backpacktravelAlliancesTx, uTxDBName, accountID, sTableName, lastLedgerSequence, tx.getTransactionID(), eachTx);
	...
	TER TableStorage::TableStorageHandlePut(BackpacktravelAlliancesTx& transactor,uint160 uTxDBName, AccountID accountID,std::string sTableName,uint32 lastLedgerSequence,uint256 txhash, STTx const & tx)
    {
        std::lock_guard<std::mutex> lock(mutexMap_);
