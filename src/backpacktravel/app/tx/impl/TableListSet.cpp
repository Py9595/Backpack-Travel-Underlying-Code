...
#include <backpacktravel/app/tx/impl/Tuning.h>
#include <backpacktravel/app/tx/OperationRule.h>
...
namespace ripple {
        ...
        TableListSet::preflight(PreflightContext const& ctx)
    {
        ...
        tmpret = BackpacktravelAlliancesTx::preflight(ctx);
        if (!isTesSuccess(tmpret))
            return tmpret;
            ...
            if (tablesle)  //table exist
            {
                ...
                case T_CREATE:
                {
					...
					// Check reserve and funds availability
					{
						...
						STAmount priorBalance = STAmount((*sleAccount)[sfBalance]).bta();
	...
	std::pair<TER, std::string> TableListSet::dispose(TxStore& txStore, const STTx& tx)
	{
		...
		return BackpacktravelAlliancesTx::dispose(txStore, tx);
	}
