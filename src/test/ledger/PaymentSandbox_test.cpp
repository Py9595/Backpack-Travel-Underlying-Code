...
class PaymentSandbox_test : public beast::unit_test::suite
{
    ...
    */
    void testSelfFunding (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA (10000), snd, rcv, gw1, gw2);
    ...
    void testSubtractCredits (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA (10000), alice, gw1, gw2);
    ...
    void testReserve(std::initializer_list<uint256> fs)
    {
        ...
        auto accountFundsBTA = [&dj](
            ReadView const& view, AccountID const& id) -> BTAAmount
        {
            return toAmount<BTAAmount> (accountHolds (
                view, id, btaCurrency (), btaAccount (), fhZERO_IF_FROZEN, dj));
        };

        auto reserve = [](jtx::Env& env, std::uint32_t count) -> BTAAmount
        {
        ...
        PaymentSandbox sb (&av);
        {
            ...
            accountSend (sb, btaAccount (), alice, BTA(100), dj);
            accountSend (sb, alice, btaAccount (), BTA(100), dj);
            BEAST_EXPECT(accountFundsBTA (sb, alice) == beast::zero);
        }
    }
