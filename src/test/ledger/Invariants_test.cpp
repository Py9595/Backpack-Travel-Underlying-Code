...
class Invariants_test : public beast::unit_test::suite
{
    ...
    void
    testBTANotCreated (bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - BTA created";
        doInvariantCheck (enabled,
            {{ "BTA net change was 500 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // put a single account in the view and "manufacture" some BTA
    ...
    testTypesMatch(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "ledger entry type mismatch" },
             { "BTA net change was -1000000000 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
    ...
    void
    testNoBTATrustLine(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - trust lines with BTA not allowed";
        doInvariantCheck (enabled,
            {{ "an BTA trust line was created" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                // create simple trust SLE with bta currency
                auto index = getRippleStateIndex (A1, A2, btaIssue().currency);
    ...
    void
    testBTABalanceCheck(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - BTA balance checks";
        ...
        doInvariantCheck (enabled,
            {{ "Cannot return non-native STAmount as BTAAmount" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                //non-native balance
                auto const sle = ac.view().peek (keylet::account(A1.id()));
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account BTA balance" },
             {  "BTA net change was 99999999000000001 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account BTA balance" },
             { "BTA net change was -1000000001 on a fee of 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // balance is negative
                auto const sle = ac.view().peek (keylet::account(A1.id()));
    ...
    testNoBadOffers(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerPays, BTA(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerGets, BTA(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setAccountID (sfAccount, A1.id());
                sleNew->setFieldU32 (sfSequence, (*sle)[sfSequence]);
                sleNew->setFieldAmount (sfTakerPays, BTA(10));
                sleNew->setFieldAmount (sfTakerGets, BTA(11));
                ac.view().insert (sleNew);
                return true;
            });
    }
    ...
    testNoZeroEscrow(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "BTA net change was -1000000 on a fee of 0"},
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                auto sleNew = std::make_shared<SLE> (
                    keylet::escrow(A1, (*sle)[sfSequence] + 2));
                sleNew->setFieldAmount (sfAmount, BTA(-1));
                ac.view().insert (sleNew);
                return true;
            });

        doInvariantCheck (enabled,
            {{ "BTA net change was 100000000000000001 on a fee of 0" },
             {  "escrow specifies invalid amount" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
...
public:
    void run ()
    {
        ...
        for(auto const& b : {false, true})
        {
            ...
            testTypesMatch (b);
            testNoBTATrustLine (b);
            testBTABalanceCheck (b);
            testNoBadOffers (b);
            testNoZeroEscrow (b);
        }
    }
