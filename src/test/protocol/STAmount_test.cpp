...
public:
    ...
    {
        {
            ...
            Issue const bta (btaIssue ());

            // fractional BTA (i.e. drops)
            testSetValue ("1", bta);
            testSetValue ("22", bta);
            testSetValue ("333", bta);
            testSetValue ("4444", bta);
            testSetValue ("55555", bta);
            testSetValue ("666666", bta);

            // 1 BTA up to 100 billion, in powers of 10 (in drops)
            testSetValue ("1000000", bta);
            testSetValue ("10000000", bta);
            testSetValue ("100000000", bta);
            testSetValue ("1000000000", bta);
            testSetValue ("10000000000", bta);
            testSetValue ("100000000000", bta);
            testSetValue ("1000000000000", bta);
            testSetValue ("10000000000000", bta);
            testSetValue ("100000000000000", bta);
            testSetValue ("1000000000000000", bta);
            testSetValue ("10000000000000000", bta);
            testSetValue ("100000000000000000", bta);

            // Invalid native values:
            testSetValue ("1.1", bta, false);
            testSetValue ("100000000000000001", bta, false);
            testSetValue ("1000000000000000000", bta, false);
        }
    ...
    void testNativeCurrency ()
    {
        ...
        unexpected (to_string (Currency ()) != "BTA", "cHC(BTA)");
        Currency c;
    ...
    void testCustomCurrency ()
    {
        ...
        unexpected (multiply (STAmount (noIssue(), 20), STAmount (3), btaIssue ()).getText () != "60",
            "STAmount multiply fail 2");
        unexpected (multiply (STAmount (20), STAmount (3), noIssue()).getText () != "60",
            "STAmount multiply fail 3");
        unexpected (multiply (STAmount (20), STAmount (3), btaIssue ()).getText () != "60",
        ...
        unexpected (divide (STAmount (noIssue(), 60), STAmount (3), btaIssue ()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), noIssue()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), btaIssue ()).getText () != "20",
            "STAmount divide fail");
    ...
    void testUnderflow ()
    {
        ...
        bigDsmall = divide (smallValue, bigValue, btaIssue ());

        BEAST_EXPECT(bigDsmall == zero);

        bigDsmall = divide (smallValue, bigNative, btaIssue ());
    ...
    void testRounding ()
    {
        ...
        STAmount dripTest1 = mulRound (twoThird2, two, btaIssue (), false);
        STAmount dripTest2 = multiply (twoThird2, two, btaIssue ());
        STAmount dripTest3 = mulRound (twoThird2, two, btaIssue (), true);
        log << dripTest1;
        log << dripTest2;
        log << dripTest3;
#endif
    ...
    testConvertBTA ()
    {
        testcase ("STAmount to BTAAmount conversions");
        ...
        {
            auto const t = amountFromString (bta, std::to_string (drops));
            auto const s = t.bta ();
            BEAST_EXPECT(s.drops() == drops);
            BEAST_EXPECT(t == STAmount (BTAAmount (drops)));
            BEAST_EXPECT(s == BTAAmount (drops));
        }

        try
        {
            auto const t = amountFromString (usd, "136500");
            fail (to_string (t.bta ()));
        }
    ...
    testConvertIOU ()
    {
        ...
        try
        {
            auto const t = amountFromString (bta, "136500");
