...
public:
	...
	void init_env() {
		...
		if (npos == std::string::npos) {
			std::cout << "usage: backpacktravelalliancesd.exe --unittest=\"DataBaseConn\""
				<< "--unittest-arg=\"conf = backpacktravelalliancesd.cfg\"" << std::endl;
			exit(-1);
		}

		try {
			backpacktravelalliances_conf_ = args.substr(npos + 4 + 1);
			std::cout << "conf = " << backpacktravelalliances_conf_ << std::endl;

			ripple::Config config;
			config.setup(backpacktravelalliances_conf_, true, false, false);
	...
	void test_InsertRecord() {
		ripple::DatabaseCon database(setup_, "ripple", NULL, 0, db_type_);
		soci::session& session = database.getSession();
		session << "insert into user (id,name) values (1,'backpacktravel')";
	...
	void test_UpdateRecord() {
		ripple::DatabaseCon database(setup_, "ripple", NULL, 0, db_type_);
		soci::session& session = database.getSession();
		std::string new_name = "new backpacktravel";
		int update_id = 1;
	...
	void test_SelectRecord() {
		...
		std::string expected_name[] = {"new backpacktravel", "ripple"};
...
private:
	...
	std::string backpacktravelalliances_conf_;
	std::string db_type_;
