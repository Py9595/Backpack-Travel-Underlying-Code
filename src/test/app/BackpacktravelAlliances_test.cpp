...
struct BackpacktravelAlliances_test : public beast::unit_test::suite
{
    void run() override
    {
        testcase("BackpacktravelAlliances test");
        using namespace jtx;
        ...
        //JLOG(logs_->journal("BackpacktravelAlliances").trace()) << "1000 times of insert errortimes is"<< errortimes;
        //JLOG(logs_->journal("BackpacktravelAlliances").trace()) << "total running time is" << double(end - start) / CLOCKS_PER_SEC;
    }
};

BEAST_DEFINE_TESTSUITE(BackpacktravelAlliances,app,ripple);
