...
bool
equal(std::unique_ptr<Step> const& s1, BTAEndpointStepInfo const& btasi)
{
    if (!s1)
        return false;
    return test::btaEndpointStepEqual(*s1, btasi.acc);
}
...
ape(AccountID const& a)
{
    return STPathElement(
        STPathElement::typeAccount, a, btaCurrency(), btaAccount());
};
...
{
    return STPathElement(
        STPathElement::typeIssuer, btaAccount(), btaCurrency(), account);
};
...
{
    return STPathElement(
        STPathElement::typeCurrency, btaAccount(), c, btaAccount());
};
...
public:
    ...
    valid() const
    {
        return
            ...
            (!hasAny({SB::bta, SB::existingCur, SB::prevCur}) || has(SB::cur)) &&
            // These will be duplicates
            (count({SB::bta, SB::existingCur, SB::prevCur}) <= 1) &&
...
struct ExistingElementPool
{
        ...
        boost::optional<size_t> const& offererIndex)
    {
        ...
        // create offers to/from bta to every other ious
        for (auto const& iou : ious)
        {
            auto const owner =
                offererIndex ? accounts[*offererIndex] : iou.account;
            env(offer(owner, iou(1000), BTA(1000)), txflags(tfPassive));
            env(offer(owner, BTA(1000), iou(1000)), txflags(tfPassive));
            env.close();
        }
    }

    std::int64_t
    totalBTA(ReadView const& v, bool incRoot)
    {
        std::uint64_t totalBTA = 0;
    ...
    // Check that the balances for all accounts for all currencies & BTA are the
    // same
    bool
    checkBalances(ReadView const& v1, ReadView const& v2)
    {
        ...
        auto btaBalance = [](ReadView const& v, ripple::Keylet const& k) {
        ...
        {
            {
                ...
                auto const b1 = btaBalance(v1, ak);
                auto const b2 = btaBalance(v2, ak);
                totalBTA[0] += b1.mantissa();
                totalBTA[1] += b2.mantissa();
                if (b1 != b2)
                    diffs.emplace_back(b1, b2, btaAccount(), *ai1);
            }
            for (auto ai2 = accounts.begin(); ai2 != aie; ++ai2)
            {
                if (ai1 >= ai2)
...
struct PayStrandAllPairs_test : public beast::unit_test::suite
{
    ...
    testAllPairs(std::initializer_list<uint256> fs)
    {
        ...
        auto callback = [&](
            ...
            // check combinations of src and dst currencies (inc bta)
            // Check the results
            auto const terMatch = [&] {
                ...
                // bta followed by offer that doesn't specify both currency and
                // issuer (and currency is not bta, if specifyed)
                if (isBTA(sendMax) &&
                    !(p[0].hasCurrency() && isBTA(p[0].getCurrency())) &&
                ...
                for (size_t i = 0; i < p.size() - 1; ++i)
                {
                    ...
                    if ((tCur & STPathElement::typeCurrency) &&
                        isBTA(p[i].getCurrency()) &&
                        (tNext & STPathElement::typeAccount) &&
                        !isBTA(p[i + 1].getAccountID()))
                    {
                        return true;
                    }
                }
        ...
        for (auto const srcAmtIsBTA : {false, true})
        {
            for (auto const dstAmtIsBTA : {false, true})
            {
                for (auto const hasPrefix : {false, true})
                {
                    ...
                    STAmount const sendMax{
                        srcAmtIsBTA ? btaIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        -1,  // (-1 == no limit)
                        0};

                    STAmount const deliver{
                        dstAmtIsBTA ? btaIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        1,
                        0};
...
struct PayStrand_test : public beast::unit_test::suite
{
    ...
    testToStrand(std::initializer_list<uint256> fs)
    {
        ...
        using B = ripple::Book;
        using BTAS = BTAEndpointStepInfo;
...

        {
            ...
            {
                STPath const path = STPath({ipe(USD), cpe(btaCurrency())});
                auto r = toStrand(
                    *env.current(),
                    alice,
                    alice,
...

        {
            ...
            // Path with BTA cross currency bridged payment
            test(
                env,
                EUR,
                USD.issue(),
                STPath({cpe(btaCurrency())}),
                tesSUCCESS,
                D{alice, gw, usdC},
                B{USD, BTA},
                B{BTA, EUR},
            ...
            // BTA -> BTA transaction can't include a path
            test(env, BTA, boost::none, STPath({ape(carol)}), temBAD_PATH);
            ...
            test(
                ...
                STPath({STPathElement(
                    0, btaAccount(), btaCurrency(), btaAccount())}),
                temBAD_PATH);
        ...
        {
            Env env(*this, with_features(fs));
            env.fund(BTA(10000), alice, bob, noripple(gw));
            env.trust(USD(1000), alice, bob);
            env(pay(gw, alice, USD(100)));
...

        {
            ...
            path.emplace_back(boost::none, btaCurrency(), boost::none);
            ...
            auto r = toStrand(
                *env.current(),
                ...
                env.app().logs().journal("Flow"));
            BEAST_EXPECT(r.first == tesSUCCESS);
            BEAST_EXPECT(equal(r.second, D{alice, gw, usdC}, B{USD.issue(), btaIssue()}, BTAS{bob}));
        }
    }
    ...
    testRIPD1373(std::initializer_list<uint256> fs)
    {
        ...
        if (hasFeature(fix1373, fs))
        {
            ...
            env(offer(bob, BTA(100), bob["USD"](100)), txflags(tfPassive));
            env(offer(gw, BTA(100), USD(100)), txflags(tfPassive));
...

        {
            ...
            env(offer(bob, BTA(100), USD(100)), txflags(tfPassive));
            env(offer(bob, USD(100), BTA(100)), txflags(tfPassive));

            // payment path: BTA -> BTA/USD -> USD/BTA
            env(pay(alice, carol, BTA(100)),
                path(~USD, ~BTA),
                txflags(tfNoRippleDirect),
                ter(temBAD_SEND_BTA_PATHS));
        }
    ...
    testLoop(std::initializer_list<uint256> fs)
    {
...

        {
            ...
            // payment path: USD -> USD/BTA -> BTA/USD
            env(pay(alice, carol, USD(100)),
                sendmax(USD(100)),
                path(~BTA, ~USD),
                txflags(tfNoRippleDirect),
        ...
        }
        {
            ...
            // payment path: BTA->BTA/USD->USD/EUR->USD/CNY
