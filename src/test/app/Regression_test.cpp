...
struct Regression_test : public beast::unit_test::suite
{
    ...
    void testOffer1()
    {
        ...
        env(offer("alice", USD(10), BTA(10)), require(owners("alice", 1)));
        env(offer("alice", USD(20), BTA(10)), json(R"raw(
                { "OfferSequence" : 2 }
    ...
    {
        testcase("Account balance < fee destroys correct amount of BTA");
        using namespace jtx;
        Env env(*this);
        env.memoize("alice");
        ...
        auto const aliceBTA = 400;
        auto const aliceAmount = BTA(aliceBTA);
        ...
        }
        {
            ...
            BEAST_EXPECT(balance == BTA(0));
        }
        expectedDrops -= aliceBTA * dropsPerBTA<int>::value;
        BEAST_EXPECT(next->info().drops == expectedDrops);
    }
    ...
    void testSecp256r1key ()
    {
        ...
        env.fund(BTA(10000), alice, becky);
