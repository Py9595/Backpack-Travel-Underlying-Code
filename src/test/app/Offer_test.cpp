...
class Offer_test : public beast::unit_test::suite
{
    ...
    BTAAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    static auto
    btaMinusFee (jtx::Env const& env, std::int64_t btaAmount)
        -> jtx::PrettyAmount
    {
        using namespace jtx;
        auto feeDrops = env.current()->fees().base;
        return drops (dropsPerBTA<std::int64_t>::value * btaAmount - feeDrops);
    };
...
public:
    void testRmFundedOffer (std::initializer_list<uint256> fs)
    {
        ...
        // be "taker gets" BTA. Old, broken would remove the first
        // "taker gets" bta offer, even though the offer is still funded and
        ...
        env.fund (BTA (10000), alice, bob, carol, gw);
        ...
        // "taker gets" must be BTA
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), BTA (49)));
        env (offer (carol, BTC (51), BTA (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, BTA (50), USD (50)));
        env (offer (carol, BTA (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (BTA, USD), Path (USD));
        ...
        BEAST_EXPECT(!isOffer (env, carol, BTC (1), USD (100)) &&
            isOffer (env, carol, BTC (49), BTA (49)));
    }
    ...
    void testCanceledOffer (std::initializer_list<uint256> fs)
    {
        ...
        env (offer (alice, BTA (500), USD (100)),
            require (offers (alice, 1)));

        BEAST_EXPECT(isOffer (env, alice, BTA (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        env (offer (alice, BTA (300), USD (100)),
             json (jss::OfferSequence, firstOfferSeq),
             require (offers (alice, 1)));

        BEAST_EXPECT(isOffer (env, alice, BTA (300), USD (100)) &&
            !isOffer (env, alice, BTA (500), USD (100)));

        // Test canceling non-existent offer.
        env (offer (alice, BTA (400), USD (200)),
             json (jss::OfferSequence, firstOfferSeq),
             require (offers (alice, 2)));

        BEAST_EXPECT(isOffer (env, alice, BTA (300), USD (100)) &&
            isOffer (env, alice, BTA (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const nextOfferSeq = env.seq (alice);
        env (offer (alice, BTA (222), USD (111)),
            require (offers (alice, 3)));

        BEAST_EXPECT(isOffer (env, alice, BTA (222), USD (111)));
    ...
    void testBTATinyPayment (std::initializer_list<uint256> fs)
    {
        testcase ("BTA Tiny payments");

        // Regression test for tiny bta payments
        // In some cases, when the payment code calculates
        // the amount of bta needed as input to an bta->iou offer
        ...
        // 1) When we calculate the amount of input bta needed for an offer
        //    from bta->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (BTA (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforceNoRipple (std::initializer_list<uint256> fs)
    {
...

        {
            ...
            env.fund (BTA (10000), alice, noripple (bob), carol, dan, gw1, gw2);
            ...
            env (offer (dan, BTA (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (BTA (50)), txflags (tfNoRippleDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (BTA (10000), alice, bob, carol, dan, gw1, gw2);
            ...
            env.require (balance (alice, btaMinusFee (env, 10000 - 50)));
    ...
    testInsufficientReserve (std::initializer_list<uint256> fs)
    {
        ...
        // No crossing:
        {
            ...
            env (offer (alice, btaOffer, usdOffer),
        ...
        // Partial cross:
        {
            ...
            auto const btaOffer2 = BTA (500);

            env.fund (r + f + btaOffer, bob);
            env (offer (bob, usdOffer2, btaOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, btaOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + btaOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + btaOffer2),
        ...
        // buy BTA. If it fully crosses, we succeed.
        {
            Env env {*this, with_features (fs)};
            auto const closeTime =
                fix1449Time () +
            ...
            env.fund (r + f + btaOffer, bob, carol);
            env (offer (bob, usdOffer2, btaOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, btaOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + btaOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + btaOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + btaOffer2),
    ...
    testFillModes (std::initializer_list<uint256> fs)
    {
        ...
        auto const startBalance = BTA (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
        ...
        // a fee and not place the offer on the books:
        {
            ...
            env (offer (alice, BTA (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + BTA (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - BTA (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), BTA (50)),            ter(tesSUCCESS));
            env (offer (alice, BTA (1000), USD (1000)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + BTA (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - BTA (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), BTA (50)),            ter(tesSUCCESS));
            env (offer (alice, BTA (50), USD (50)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + BTA (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - BTA (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), BTA(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == BTA (2000));
            ...
            env (offer (bob, BTA(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == BTA (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, BTA(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), BTA(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), BTA(1001)));
            env.close();

            env (offer ("alice", USD(500), BTA(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == BTA (999));
            }
        }
    }
    ...
    testMalformed(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (alice, USD (1000), BTA (1000)),
        ...
        {
            // Alice tries an BTA to BTA order:
            env (offer (alice, BTA (1000), BTA (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), BTA (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use BTA as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, BTA (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (bob, usdOffer, btaOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(std::initializer_list<uint256> fs)
    {
        ...
        env (offer ("alice", usdOffer, btaOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, btaOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + BTA(1), "carol");
        env (offer ("carol", usdOffer, btaOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + BTA(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, btaOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + btaOffer, "eve");
        env (offer ("eve", usdOffer, btaOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + btaOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA (10000), gw);
        if (use_partner)
        {
            env.fund (BTA (10000), partner);
        ...
        // through BTA
        env (offer (account_to_test, BTC (250), BTA (1000)),
                 offers (account_to_test, 1));

        // validate that the book now shows a BTC for BTA offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTC(250), BTA(1000)));

        auto const secondLegSeq = env.seq(account_to_test);
        env (offer (account_to_test, BTA(1000), USD (50)),
                 offers (account_to_test, 2));

        // validate that the book also shows a BTA for USD offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTA(1000), USD(50)));
        ...
        // behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == BTA (0));
            }
        }
    ...
    testNegativeBalance(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (alice, USD (50), BTA (150000)));
        ...
        // small_amount and transfer no BTA.  The new offer crossing transfers
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, btaIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, btaIssue()) ==
            bob_initial_balance -
    ...
    void
    testOfferCrossWithBTA(bool reverse_order, std::initializer_list<uint256> fs)
    {
        testcase (std::string("Offer Crossing with BTA, ") +
        ...
        env.fund (BTA (10000), gw, alice, bob);
        ...
        if (reverse_order)
            env (offer (bob, USD (1), BTA (4000)));

        env (offer (alice, BTA (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                BTA (10000).value ().mantissa () -
                BTA (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                BTA (100000).value ().mantissa () -
                BTA (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (env.master, BTA (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire(std::initializer_list<uint256> fs)
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BTA (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == USD (100).value ().getJson (0));

        env (pay (alice, alice, BTA (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionIntoDebt(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA (10000), alice, bob, carol);
    ...
    testCurrencyConversionInParts(std::initializer_list<uint256> fs)
    {
        ...
        env (pay (alice, alice, BTA (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 BTA
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BTA (300).value ().getText ());
        BEAST_EXPECT(
        ...
        // alice now has 200 more BTA from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                BTA (10000).value ().mantissa () +
                BTA (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to BTA which should fail
        // due to PartialPayment.
        env (pay (alice, alice, BTA (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to BTA, should succeed because
        ...
        // only 300 BTA should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartBTA(std::initializer_list<uint256> fs)
    {
        testcase ("Cross Currency Payment: Start with BTA");
        ...
        env.fund (BTA (10000), gw, alice, bob, carol);
        ...
        env (offer (carol, BTA (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (BTA (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == BTA (250).value ().getText ());
    }

    void
    testCrossCurrencyEndBTA(std::initializer_list<uint256> fs)
    {
        testcase ("Cross Currency Payment: End with BTA");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), BTA (500)));

        env (pay (alice, bob, BTA (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                BTA (10000).value ().mantissa () +
                BTA (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BTA (250).value ().getText ());
        BEAST_EXPECT(
    ...
    testCrossCurrencyBridged(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, BTA (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "BTA";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BTA (200).value ().getText ());
        BEAST_EXPECT(
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == BTA (200).value ().getText ());
    }
    ...
    testOfferFeesConsumeFunds(std::initializer_list<uint256> fs)
    {
        ...
        //  1 for payment          == 4
        auto const starting_bta = BTA (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_bta, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, BTA (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), BTA (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == BTA (350).value ().getText ()
        );
    ...
    testSellFlagBasic(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (starting_bta, gw, alice, bob);
        ...
        env (offer (bob, BTA (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), BTA (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == BTA (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit(std::initializer_list<uint256> fs)
    {
        ...
        env (offer (bob, BTA (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 BTA.
        // Selling BTA.
        // Will sell all 100 BTA and get more USD than asked for.
        env (offer (alice, USD (100), BTA (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(std::initializer_list<uint256> fs)
    {
        ...
        auto const starting_bta = BTA (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
    ...
    void testPartialCross (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for BTA : USD.
        enum preTrustType {noPreTrust, gwPreTrust, acctPreTrust};
        struct TestData
        {
            std::string account;       // Account operated on
            STAmount fundBta;          // Account funded with
            int bookAmount;            // USD -> BTA offer on the books
            preTrustType preTrust;     // If true, pre-establish trust line
            int offerAmount;           // Account offers this much BTA -> USD
            TER tec;                   // Returned tec code
            STAmount spentBta;         // Amount removed from fundBta
        ...
        {
//acct                             fundBta  bookAmt      preTrust  offerAmt                     tec       spentBta    balanceUSD  offers  owners
...
{"flo",             reserve (env, 1) + 0*f,       1,   noPreTrust,     1000,             tesSUCCESS, BTA(   1) + f, USD(      1),      0,     1},
{"gay",             reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BTA(  50) + f, USD(     50),      0,     1},
{"hye", BTA(1000)                    + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BTA( 800) + f, USD(    800),      0,     1},
{"ivy", BTA(   1) + reserve (env, 1) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, BTA(   1) + f, USD(      1),      0,     1},
{"joy", BTA(   1) + reserve (env, 2) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, BTA(   1) + f, USD(      1),      1,     2},
{"kim", BTA( 900) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BTA( 999) + f, USD(    999),      0,     1},
{"liz", BTA( 998) + reserve (env, 0) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BTA( 998) + f, USD(    998),      0,     1},
{"meg", BTA( 998) + reserve (env, 1) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BTA( 999) + f, USD(    999),      0,     1},
{"nia", BTA( 998) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BTA( 999) + f, USD(    999),      1,     2},
{"ova", BTA( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BTA( 999) + f, USD(    999),      0,     1},
{"pam", BTA( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BTA(1000) + f, USD(   1000),      0,     1},
{"rae", BTA( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BTA(1000) + f, USD(   1000),      0,     1},
{"sue", BTA(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"eli", BTA(  20) + reserve (env, 0) + 1*f,    1000,   gwPreTrust,     1000,             tesSUCCESS, BTA(20) + 1*f, USD(     20),      0,     0},
{"fyn",             reserve (env, 1) + 0*f,       0,   gwPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
{"gar",             reserve (env, 1) + 0*f,       1,   gwPreTrust,     1000,             tesSUCCESS, BTA(   1) + f, USD(      1),      1,     1},
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, BTA(   1) + f, USD(      1),      1,     1},
...
{"ted", BTA(  20) + reserve (env, 1) + 2*f,    1000, acctPreTrust,     1000,             tesSUCCESS, BTA(20) + 2*f, USD(     20),      0,     1},
{"uli",             reserve (env, 2) + 0*f,       0, acctPreTrust,     1000, tecINSUF_RESERVE_OFFER,           2*f, USD(      0),      0,     1},
{"vic",             reserve (env, 2) + 0*f,       1, acctPreTrust,     1000,             tesSUCCESS, BTA( 1) + 2*f, USD(      1),      0,     1},
{"wes",             reserve (env, 2) + 1*f,       0, acctPreTrust,     1000,             tesSUCCESS,           2*f, USD(      0),      1,     2},
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, BTA( 1) + 2*f, USD(      1),      1,     2},
};

        for (auto const& t : tests)
        {
            auto const acct = Account(t.account);
            env.fund (t.fundBta, acct);
            env.close();
            ...
            if (book)
                env (offer (gw, BTA (book), USD (book)));
            env.close();
            ...
            // to be enough BTA reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), BTA (acctOffer)), ter (t.tec));
            env.close();
            std::uint32_t const acctOfferSeq = env.seq (acct) - 1;

            BEAST_EXPECT (env.balance (acct, USD.issue()) == t.balanceUsd);
            BEAST_EXPECT (
                env.balance (acct, btaIssue()) == t.fundBta - t.spentBta);
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == BTA (leftover));
    ...
    void
    testBTADirectCross (std::initializer_list<uint256> fs)
    {
        testcase ("BTA Direct Crossing");
        ...
        // The scenario:
        //   o alice has USD but wants BTA.
        //   o bob has BTA but wants USD.
        auto const alicesBTA = env.balance (alice);
        auto const bobsBTA = env.balance (bob);

        env (offer (alice, btaOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, btaOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesBTA + btaOffer - fee),
            balance (bob,   bobsBTA   - btaOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), BTA(999)));
        env (offer (bob, btaOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == BTA (1));
        }
   }
    ...
    testBridgedCross (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA(1000000), gw, alice, bob, carol);
        env.close();
        ...
        env (offer (alice, BTA(1000), usdOffer));
        env (offer (bob, eurOffer, BTA(1000)));
        auto const bobBtaBalance = env.balance (bob);
        env.close();
        ...
        env.require (
            ...
            balance (bob, bobBtaBalance - BTA(400)),
            offers (carol, 0));
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == BTA (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == BTA (600));
        ...
        env.require (
            ...
            balance (bob, bobBtaBalance - BTA(1000)),
            offers (bob, 0),
            offers (carol, 0));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == BTA (0));
        }
    }
    ...
    testSellOffer (std::initializer_list<uint256> fs)
    {
        ...
        {
            std::string account;       // Account operated on
            STAmount fundBta;          // BTA acct funded with
            ...
            TestData (
                ...
                STAmount const& fundBta_,       // BTA acct funded with
                ...
                STAmount const& spentBta_,      // Amount removed from fundBta
            ...
            TestData (
                ...
                int offers_,                    // Offers on acct
                int owners_)                    // Owners on acct
                : TestData (std::move(account_), fundBta_, fundUSD_, gwGets_,
                  gwPays_, acctGets_, acctPays_, tec_, spentBta_, finalUsd_,
        ...
        {
// acct pays BTA
//acct                           fundBta  fundUSD   gwGets   gwPays  acctGets  acctPays                      tec         spentBta  finalUSD  offers  owners  takerGets  takerPays
{"ann", BTA(10) + reserve (env, 0) + 1*f, USD( 0), BTA(10), USD( 5),  USD(10),  BTA(10), tecINSUF_RESERVE_OFFER, BTA( 0) + (1*f),  USD( 0),      0,     0},
{"bev", BTA(10) + reserve (env, 1) + 1*f, USD( 0), BTA(10), USD( 5),  USD(10),  BTA(10),             tesSUCCESS, BTA( 0) + (1*f),  USD( 0),      1,     1,     BTA(10),  USD(10)},
{"cam", BTA(10) + reserve (env, 0) + 1*f, USD( 0), BTA(10), USD(10),  USD(10),  BTA(10),             tesSUCCESS, BTA(10) + (1*f),  USD(10),      0,     1},
{"deb", BTA(10) + reserve (env, 0) + 1*f, USD( 0), BTA(10), USD(20),  USD(10),  BTA(10),             tesSUCCESS, BTA(10) + (1*f),  USD(20),      0,     1},
{"eve", BTA(10) + reserve (env, 0) + 1*f, USD( 0), BTA(10), USD(20),  USD( 5),  BTA( 5),             tesSUCCESS, BTA( 5) + (1*f),  USD(10),      0,     1},
{"flo", BTA(10) + reserve (env, 0) + 1*f, USD( 0), BTA(10), USD(20),  USD(20),  BTA(20),             tesSUCCESS, BTA(10) + (1*f),  USD(20),      0,     1},
{"gay", BTA(20) + reserve (env, 1) + 1*f, USD( 0), BTA(10), USD(20),  USD(20),  BTA(20),             tesSUCCESS, BTA(10) + (1*f),  USD(20),      0,     1},
{"hye", BTA(20) + reserve (env, 2) + 1*f, USD( 0), BTA(10), USD(20),  USD(20),  BTA(20),             tesSUCCESS, BTA(10) + (1*f),  USD(20),      1,     2,     BTA(10),  USD(10)},
// acct pays USD
{"meg",           reserve (env, 1) + 2*f, USD(10), USD(10), BTA( 5),  BTA(10),  USD(10), tecINSUF_RESERVE_OFFER, BTA(  0) + (2*f),  USD(10),      0,     1},
{"nia",           reserve (env, 2) + 2*f, USD(10), USD(10), BTA( 5),  BTA(10),  USD(10),             tesSUCCESS, BTA(  0) + (2*f),  USD(10),      1,     2,     USD(10),  BTA(10)},
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), BTA(10),  BTA(10),  USD(10),             tesSUCCESS, BTA(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), BTA(20),  BTA(10),  USD(10),             tesSUCCESS, BTA(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), BTA(40),  BTA(10),  USD(10),             tesSUCCESS, BTA(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), BTA( 5),  BTA(10),  USD(10),             tesSUCCESS, BTA( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  BTA( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), BTA(10),  BTA(10),  USD(10),             tesSUCCESS, BTA(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  BTA( 5)},
};
    ...
    testSellWithFillOrKill (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA(10000000), gw, alice, bob);

        // bob offers BTA for USD.
        env (trust(bob, USD(200)));
        env.close();
        env (pay(gw, bob, USD(100)));
        env.close();
        env (offer(bob, BTA(2000), USD(20)));
        env.close();
        {
            ...
            env (offer(alice, USD(21), BTA(2100), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(alice, USD(20), BTA(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, BTA(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), BTA(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for BTA(500), USD(5)
            env (offer(alice, USD(1), BTA(501), tfSell | tfFillOrKill));
            env.close();
            env.require (balance (alice, USD(35)));
            env.require (offers (alice, 0));
            env.require (balance (bob, USD(65)));
        }
        {
            ...
            // We're using bob's left-over offer for BTA(500), USD(5)
            env (offer(alice, USD(1), BTA(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (std::initializer_list<uint256> fs)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (BTA(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for BTA.  alice takes bob's offer.
            ...
            env (offer (bob, BTA(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), BTA(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, BTA( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, BTA(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell BTA
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (BTA(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), BTA(1)));
            env.close();

            env (offer (deb, BTA(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, BTA( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, BTA(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (BTA(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), BTA(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, BTA(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, BTA(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == BTA (2000));
            ...
            env.require (balance (fyn, BTA(22000)));
        ...
        env.fund (BTA(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove BTA from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (BTA(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has BTA but wants EUR.
            ...
            env (offer (ova, BTA(2), USD(100)));
            env (offer (pat, EUR(100), BTA(2)));
            env.close();
            ...
            env.require (balance (ova, BTA(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (std::initializer_list<uint256> fs)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == BTA (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == BTA (1000));
        }
    }
    ...
    testSelfCrossOffer2 (std::initializer_list<uint256> fs)
    {
        ...
        TestData const tests[]
        {
// acct               fundBTA    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (std::initializer_list<uint256> fs)
    {
        ...
        env.fund(BTA(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), BTA(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, BTA(50000), USD(5000)));
        env.close();

        // alice's offer should have been removed, since she's down to her
        // BTA reserve.
        env.require (balance (alice, BTA(250)));
        ...
        for (auto const& offerPtr : bobOffers)
        {
            ...
            BEAST_EXPECT(offer[sfTakerPays] == BTA (250));
        }
    }
    ...
    void testSelfCrossLowQualityOffer (std::initializer_list<uint256> fs)
    {
        ...
        // for more BTA than the offering account held.  This unit test
    ...
    void testTinyOffer (std::initializer_list<uint256> fs)
    {
        ...
        auto const CNY = gw["CNY"];
        auto const fee = env.current()->fees().base;
        auto const startBtaBalance = drops (400000000000) + (fee * 2);

        env.fund (startBtaBalance, gw, alice, bob);
        env.close();
        ...
        env.require (balance (alice, startBtaBalance - fee - drops(1)));
        env.require (balance (bob, bobsCnyStartBalance - alicesCnyOffer));
        env.require (balance (bob, startBtaBalance - (fee * 2) + drops(1)));
    }
    ...
    void testSelfPayXferFeeOffer (std::initializer_list<uint256> fs)
    {
        ...
        //
        //  2. alice makes an offer to buy BTA and sell USD.
        //  3. bob makes an offer to buy BTC and sell BTA.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells BTA to...
        //  o alice's first offer which takes bob's BTA and sells USD to...
        ...
        auto const BTC = gw["BTC"];
        auto const USD = gw["USD"];
        auto const startBtaBalance = BTA (4000000);

        env.fund (startBtaBalance, gw);
        env.close();
        ...
        struct Actor
        {
            ...
            PrettyAmount bta;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(3999999999950), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (BTA (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     BTA(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, BTA(100000), USD(1000),   tfPassive));
            env.close();
            ...
            for (auto const& actor : t.actors)
            {
                ...
                env.require (balance (actor.acct, actor.bta));
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(3999999999950), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), BTA(4000)));
        env.close();
        ...
        env (offer (bob, BTA(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (std::initializer_list<uint256> fs)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), BTA(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), BTA(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    void testRCSmoketest(std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();
    ...
    void testSelfAuth (std::initializer_list<uint256> fs)
    {
        ...
        env.fund (BTA(400000), gw, alice);
        env.close();

        // Test that gw can create an offer to buy gw's currency.
        env (offer (gw, gwUSD(40), BTA(4000)));
        env.close();
        std::uint32_t const gwOfferSeq = env.seq (gw) - 1;
        env.require (offers (gw, 1));
        ...
        env (offer (alice, BTA(4000), gwUSD(40)));
        env.close();
    ...
    void run ()
    {
        auto testAll = [this](std::initializer_list<uint256> fs) {
            ...
            testOfferCrossWithBTA(true, fs);
            testOfferCrossWithBTA(false, fs);
            ...
            testCrossCurrencyStartBTA(fs);
            testCrossCurrencyEndBTA(fs);
