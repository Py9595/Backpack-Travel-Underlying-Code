...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    void
    testBTADiscrepancy (std::initializer_list<uint256> fs)
    {
        testcase ("Discrepancy test : BTA Discrepancy");
        ...
        env.fund(BTA(2000), A1);
        env.fund(BTA(1000), A2, A6, A7);
        env.fund(BTA(5000), A3);
        env.fund(BTA(1000000), A4);
        env.fund(BTA(600000), A5);
        env.close();
        ...
        env(offer(A4, BTA(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), BTA(80086)));
        env(offer(A7, BTA(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths {
            test::Path {A2["JPY"], A2},
            test::Path {BTA, A2["JPY"], A2},
            test::Path {A6, BTA, A2["JPY"], A2} };
...
public:
    ...
    {
        testBTADiscrepancy ({});
        testBTADiscrepancy ({featureFlow});
        testBTADiscrepancy ({featureFlow, fix1373});
        testBTADiscrepancy ({featureFlow, fix1373, featureFlowCross});
    }
};
