...
public:
    void test_noReserve()
    {
...

        {
            ...
            env(pay(env.master, alice, BTA(151)));
            env.close();
            env(smallSigners);
            env.close();
        ...
        }
        {
            ...
            env(pay(env.master, alice, BTA(350)));
            env.close();
            env(bigSigners);
            env.close();
...
//      env(signers(alice, 1, { { bogie, 0x10000} }), ter (temBAD_WEIGHT));
    ...
    void test_masterSigners()
    {
        ...
        env.fund(BTA(1000), alice, becky, cheri);
        env.close();
    ...
    void test_heterogeneousSigners()
    {
        ...
        env.fund(BTA(1000), alice, becky, cheri, daria);
        env.close();
    ...
    void test_txTypes()
    {
        ...
        env.fund(BTA(1000), alice, becky, zelda, gw);
        env.close();
        ...
        auto const baseFee = env.current()->fees().base;
        std::uint32_t aliceSeq = env.seq (alice);
        env(pay(alice, env.master, BTA(1)),
