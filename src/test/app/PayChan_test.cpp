...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.bta ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, BTA (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == BTA (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == BTA (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, BTA (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - BTA (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == BTA (0));
        BEAST_EXPECT (chanAmt == BTA (2000));

        {
            // bad amounts (non-bta, negative amounts)
        ...
        // invalid account
        env (create (alice, "noAccount", BTA (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, BTA (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), BTA (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, BTA (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-bta)
            auto const iou = USDA (100).value ();
            auto const negBTA = BTA (-100).value ();
            auto const posBTA = BTA (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posBTA, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posBTA), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negBTA, negBTA), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posBTA, negBTA), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negBTA, posBTA), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = BTA (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + BTA (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + BTA (500);
        ...
        env (fund (bob, chan, BTA (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, BTA (1500));
            env (claim (bob, chan, BTA (1500).value (), BTA (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testCancelAfter ()
    {
        ...
        auto const carol = Account ("carol");
        {
            ...
            auto const channelFunds = BTA (1000);
            env (create (
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, BTA (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, BTA (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, BTA (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    {
        // auth amount defaults to balance if not present
        testcase ("Disallow BTA");
        using namespace jtx;
        using namespace std::literals::chrono_literals;
        {
            // Create a channel where dst disallows BTA
            Env env (*this);
            auto const alice = Account ("alice");
            auto const bob = Account ("bob");
            env.fund (BTA (10000), alice, bob);
            env (fset (bob, asfDisallowBTA));
            auto const pk = alice.pk ();
            auto const settleDelay = 3600s;
            auto const channelFunds = BTA (1000);
            env (create (alice, bob, channelFunds, settleDelay, pk),
        ...
        {
            // Claim to a channel where dst disallows BTA
            // (channel is created before disallow bta is set)
            Env env (*this);
            auto const alice = Account ("alice");
            auto const bob = Account ("bob");
            env.fund (BTA (10000), alice, bob);
            ...
            env (fset (bob, asfDisallowBTA));
            auto const preBob = env.balance (bob);
            auto const reqBal = BTA (500).value();
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, BTA (1000), settleDelay, pk);
