...
public:
    void testQueue()
    {
        ...
        env.fund(BTA(50000), noripple(alice, bob, charlie, daria));
        ...
        env.fund(BTA(50000), noripple(elmo, fred, gwen, hank));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(BTA(50000), noripple(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(BTA(1000), noripple(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(BTA(1000), noripple(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(BTA(1000), noripple(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(BTA(1000), noripple(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(BTA(1000), noripple(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(BTA(1000), noripple(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, BTA(-1000)),
            ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(BTA(100000)),
            ter(terINSUF_FEE_B));
    }
    ...
    void testMultiTxnPerAccount()
    {
        ...
        env.fund(BTA(500000), noripple(bob, charlie, daria));
        ...
        aliceFee = env.le(alice)->getFieldAmount(sfBalance).bta().drops()
            - (59);
        env(noop(alice), seq(aliceSeq),
        ...
        // Alice is broke
        env.require(balance(alice, BTA(0)));
    ...
    void testDisabled()
    {
        ...
        env.fund(BTA(50000), noripple(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(BTA(50000), noripple(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more BTA, and it'll process.
        */
    ...
    void testBlockers()
    {
        ...
        env.fund(BTA(50000), noripple(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(BTA(50000), noripple(alice, charlie), gw);
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        //////////////////////////////////////////
        // Offer with high BTA out blocks later txs
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, BTA(50000)),
            owners(alice, 0));

        // If this offer crosses, all of alice's
        // BTA will be taken (except the reserve).
        env(offer(alice, BUX(5000), BTA(50000)),
            queued);
        ...
        // has plenty of BTA, because the offer didn't
        // cross (of course).
        env.require(balance(alice, aliceBal - drops(20)),
            owners(alice, 1));

        //////////////////////////////////////////
        // Offer with low BTA out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 8, 5, 4, 256);
        aliceSeq = env.seq(alice);
        ...
        // of alice's BTA will be taken.
        env(offer(alice, BUX(50), BTA(500)),
            queued);
        ...
        //////////////////////////////////////////
        // Large BTA payment blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 10, 6, 5, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, BTA(50000)),
            queued);
        ...
        //////////////////////////////////////////
        // Small BTA payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 12, 7, 6, 256);
        ...
        env.require(balance(alice, aliceBal - BTA(500) - drops(20)),
            owners(alice, 2));
        ...
        // alice's BTA balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        ...
        // BTA balance, but her USD balance went to charlie.
        env.require(balance(alice, aliceBal - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Large BTA to IOU payment blocks later txs.

        env(offer(gw, BTA(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire BTA balance to charlie in the
        // form of USD.
        BEAST_EXPECT(BTA(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)),
            sendmax(BTA(60000)), queued);
        ...
        // to charlie using only a portion of her BTA balance
        env.require(balance(alice, aliceBal - BTA(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small BTA to IOU payment allows later txs.
        ...
        // a portion of her BTA balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > BTA(6001));
        env(pay(alice, charlie, USD(500)),
            sendmax(BTA(6000)), queued);
        ...
        // So once we close the ledger, alice sent a payment
        // to charlie using only a portion of her BTA balance
        env.require(balance(alice, aliceBal - BTA(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(BTA(500000), noripple(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(BTA(50000), noripple(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(BTA(50000), noripple(e,f,g,h,i));
    ...
    void testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(BTA(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(BTA(1)), seq(aliceSeq++), ter(terQUEUED));
