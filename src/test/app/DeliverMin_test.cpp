...
public:
    ...
    test_convert_all_of_an_asset(std::initializer_list<uint256> fs)
    {
...

        {
            ...
            env(pay("alice", "bob", USD(10)), delivermin(BTA(5)),
                txflags(tfPartialPayment),                          ter(temBAD_AMOUNT));
            ...
            env(offer("carol", BTA(5), USD(5)));
            env(pay("alice", "bob", USD(10)), paths(BTA),
                delivermin(USD(7)), txflags(tfPartialPayment),
                sendmax(BTA(5)),                                   ter(tecPATH_PARTIAL));
            env.require(balance("alice", BTA(9999.99999)));
...

        {
            ...
            env(pay("alice", "alice", USD(10000)), paths(BTA),
...

        {
            ...
            env(offer("bob", BTA(100), USD(100)));
            env(offer("bob", BTA(1000), USD(100)));
            env(offer("bob", BTA(10000), USD(100)));
            env(pay("alice", "carol", USD(10000)), paths(BTA),
                delivermin(USD(200)), txflags(tfPartialPayment),
                sendmax(BTA(1000)),                                 ter(tecPATH_PARTIAL));
            env(pay("alice", "carol", USD(10000)), paths(BTA),
                delivermin(USD(200)), txflags(tfPartialPayment),
                sendmax(BTA(1100)));
