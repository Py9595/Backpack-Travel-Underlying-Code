...
auto IPE(Issue const& iss)
{
    return STPathElement (
        ...
        btaAccount (), iss.currency, iss.account);
};
...
public:
    ...
    path_find_consume_all()
    {
...

        {
            ...
            env(offer("carol", BTA(100), USD(100)));
            ...
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["AUD"](-1),
                    boost::optional<STAmount>(BTA(100000000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["USD"](-1),
                    boost::optional<STAmount>(BTA(100000000)));
            BEAST_EXPECT(sa == BTA(100));
    ...
    alternative_path_consume_both()
    {
        ...
        env.fund(BTA(10000), "alice", "bob", gw, gw2);
        env.trust(USD(600), "alice");
    ...
    alternative_paths_limit_returned_paths_to_best_quality()
    {
        ...
        auto const gw2 = Account("gateway2");
        auto const gw2_USD = gw2["USD"];
        env.fund(BTA(10000), "alice", "bob", "carol", "dan", gw, gw2);
        env(rate("carol", 1.1));
    ...
    issues_path_negative_ripple_client_issue_23_larger()
    {
        ...
        using namespace jtx;
        Env env(*this);
        env.fund(BTA(10000), "alice", "bob", "carol", "dan", "edward");
    ...
    // carol holds gateway AUD, sells gateway AUD for BTA
    // bob will hold gateway AUD
    // alice pays bob gateway AUD using BTA
    void
    via_offers_via_gateway()
    {
        ...
        env(offer("carol", BTA(50), AUD(50)));
        env(pay("alice", "bob", AUD(10)), sendmax(BTA(100)), paths(BTA));
    ...
    void path_find_01()
    {
        testcase("Path Find: BTA -> BTA and BTA -> IOU");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(BTA(100000), A1);
        env.fund(BTA(10000), A2);
        env.fund(BTA(1000), A3, G1, G2, G3, M1);
        env.close();
        ...
        env(offer(M1, BTA(10000), G3["ABC"](1000)));

        STPathSet st;
        STAmount sa, da;

        {
            auto const& send_amt = BTA(10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, btaCurrency());
    ...
    void path_find_02()
    {
        testcase("Path Find: non-BTA -> BTA");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(BTA(1000), A1, A2, G3);
        env.fund(BTA(11000), M1);
        env.close();
        ...
        env(offer(M1, G3["ABC"](1000), BTA(10000)));
        ...
        BEAST_EXPECT(same(st, stpath(G3, IPE(btaIssue()))));
    }
    ...
    void path_find_03()
    {
        ...
        env.fund(BTA(4999.999898), SRC);
        env.fund(BTA(10846.168060), GATEWAY_DST);
        env.fund(BTA(4291.430036), MONEY_MAKER_1);
        env.fund(BTA(106839.375770), MONEY_MAKER_2);
        env.fund(BTA(1240.997150), A1);
        env.fund(BTA(14115.046893), A2);
        env.fund(BTA(512087.883181), A3);
        env.close();
        ...
        env(offer(MONEY_MAKER_2, BTA(1), GATEWAY_DST["CNY"](1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](1), BTA(1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](318000), BTA(53000)));
        env(offer(MONEY_MAKER_2, BTA(209), MONEY_MAKER_2["CNY"](4.18)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](990000), BTA(10000)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](9990000), BTA(10000)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](8870000), BTA(10000)));
        env(offer(MONEY_MAKER_2, BTA(232), MONEY_MAKER_2["CNY"](5.568)));
        env(offer(A2, BTA(2000), MONEY_MAKER_1["CNY"](66.8)));
        env(offer(A2, BTA(1200), GATEWAY_DST["CNY"](42)));
        env(offer(A2, MONEY_MAKER_1["CNY"](43.2), BTA(900)));
        env(offer(A3, MONEY_MAKER_1["CNY"](2240), BTA(50000)));
        ...
        std::tie(st, sa, da) = find_paths(env, SRC, GATEWAY_DST, send_amt,
            boost::none, btaCurrency());
        BEAST_EXPECT(equal(da, send_amt));
        BEAST_EXPECT(equal(sa, BTA(288.571429)));
    ...
    void path_find_04()
    {
        ...
        env.fund(BTA(1000), G1BS, G2SW, A1, A2);
        env.fund(BTA(11000), M1);
        env.close();
    ...
    void path_find_05()
    {
        testcase("Path Find: non-BTA -> non-BTA, same currency");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(BTA(1000), A1, A2, A3, G1, G2, G3, G4);
        env.fund(BTA(10000), A4);
        env.fund(BTA(11000), M1, M2);
        env.close();
        ...
        env(offer(M2, BTA(10000), G2["HKD"](1000)));
        env(offer(M2, G1["HKD"](1000), BTA(10000)));
...

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(M1),
                stpath(M2),
                stpath(IPE(btaIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(btaIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            //I4) BTA bridge" --
            //  Source -> AC -> OB to BTA -> OB from BTA -> AC -> Destination
            auto const& send_amt = A2["HKD"](10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, G1["HKD"].currency);
            BEAST_EXPECT(equal(da, send_amt));
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(btaIssue()), IPE(G2["HKD"]), G2)
            ));
        }
    }
    ...
    void path_find_06()
    {
        ...
        env.fund(BTA(11000), M1);
        env.fund(BTA(1000), A1, A2, A3, G1, G2);
        env.close();
