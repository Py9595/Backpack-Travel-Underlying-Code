...
inline
STAmount
toSTAmount (BTAAmount const& bta)
{
    bool const isNeg = bta.signum() < 0;
    std::uint64_t const umant = isNeg ? - bta.drops () : bta.drops ();
    return STAmount (umant, isNeg);
}

inline
STAmount
toSTAmount (BTAAmount const& bta, Issue const& iss)
{
    assert (isBTA(iss.account) && isBTA(iss.currency));
    return toSTAmount (bta);
}
...
inline
BTAAmount
toAmount<BTAAmount> (STAmount const& amt)
{
    assert (amt.mantissa () < std::numeric_limits<std::int64_t>::max ());
    bool const isNeg = amt.negative ();
    std::int64_t const sMant =
