    ...
    : STBase(name)
{
    ...
    if (isBTA (issue.currency))
        Throw<std::runtime_error> ("invalid native currency");

    issue.account.copyFrom (sit.get160 ());

    if (isBTA (issue.account))
...
STAmount::STAmount (BTAAmount const& amount)
    : mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < zero)
{
...
//
//------------------------------------------------------------------------------
BTAAmount STAmount::bta () const
{
    if (!mIsNative)
        Throw<std::logic_error> ("Cannot return non-native STAmount as BTAAmount");
...
amountFromString (Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify BTA using fractional representation
    if (isBTA(issue) && match[3].matched)
        Throw<std::runtime_error> ("BTA must be specified in integral drops.");
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    {
        if (v.isObject ())
            Throw<std::runtime_error> ("BTA may not be specified as an object");
        issue = btaIssue ();
    }
    else
    {
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isBTA (issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const bta = isBTA (issue);

    if (v1.native() && v2.native() && bta)
    {
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (bta, amount, offset);
    ...
    {
        if (bta && *stAmountCalcSwitchover2)
        {
    ...
    Issue const& issue, bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (isBTA (issue), amount, offset);

    STAmount result (issue, amount, offset, resultNegative);
    // Control when bugfixes that require switchover dates are enabled
    if (roundUp && !resultNegative && !result && *stAmountCalcSwitchover)
    {
        if (isBTA(issue) && *stAmountCalcSwitchover2)
        {
            // return the smallest value above zero
            amount = 1;
            offset = 0;
