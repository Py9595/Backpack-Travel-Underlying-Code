...
Json::Value Node::getJson () const
{
    ...
    bool const hasCurrency = !isBTA (issue_.currency);
    bool const hasAccount = !isBTA (account_);
    bool const hasIssuer = !isBTA (issue_.account);
    ...
    if (!isBTA (issue_.currency))
        jvNode[jss::currency] = to_string (issue_.currency);

    if (!isBTA (issue_.account))
