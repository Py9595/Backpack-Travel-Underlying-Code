    ...
    path::detail::FlowDebugInfo* flowDebugInfo)
{
    Issue const srcIssue = [&] {
        ...
        if (!isBTA (deliver.issue ().currency))
    ...
    const bool srcIsBTA = isBTA (srcIssue.currency);
    const bool dstIsBTA = isBTA (dstIssue.currency);

    auto const asDeliver = toAmountSpec (deliver);

    // The src account may send either bta or iou. The dst account may receive
    // either bta or iou. Since BTA and IOU amounts are represented by different
    // types, use templates to tell `flow` about the amount types.
    if (srcIsBTA && dstIsBTA)
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<BTAAmount, BTAAmount> (
                sb, strands, asDeliver.bta, partialPayment, offerCrossing,
    ...
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<IOUAmount, BTAAmount> (
                sb, strands, asDeliver.bta, partialPayment, offerCrossing,
                limitQuality, sendMax, j, flowDebugInfo));
    }

    assert (!srcIsBTA && !dstIsBTA);
    return finishFlow (sb, srcIssue, dstIssue,
