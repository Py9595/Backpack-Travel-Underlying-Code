...
The pathfinding request is identified by category, BTA to BTA, BTA to
non-BTA, non-BTA to BTA, same currency non-BTA to non-BTA, cross-currency
non-BTA to non-BTA.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isBTA(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isBTA(uSrcCurrency) ?
                btaAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isBTA(uSrcCurrency) == isBTA(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsBTA = isBTA (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsBTA && !isBTA (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsBTA ? AccountID() : account;
    ...
    bool bSrcBta = isBTA (mSrcCurrency);
    bool bDstBta = isBTA (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in BTA ";
            return false;
        }
    ...
    {
        // BTA -> BTA
        JLOG (j_.debug()) << "BTA to BTA payment";
        paymentType = pt_BTA_to_BTA;
    }
    else if (bSrcBta)
    {
        // BTA -> non-BTA
        JLOG (j_.debug()) << "BTA to non-BTA payment";
        paymentType = pt_BTA_to_nonBTA;
    }
    else if (bDstBta)
    {
        // non-BTA -> BTA
        JLOG (j_.debug()) << "non-BTA to BTA payment";
        paymentType = pt_nonBTA_to_BTA;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-BTA -> non-BTA - Same currency
        JLOG (j_.debug()) << "non-BTA to non-BTA - same currency";
        paymentType = pt_nonBTA_to_same;
    }
    else
    {
        // non-BTA to non-BTA - Different currency
        JLOG (j_.debug()) << "non-BTA to non-BTA - cross currency";
        paymentType = pt_nonBTA_to_nonBTA;
    }
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isBTA (mSrcCurrency) || (srcIssuer == mSrcAccount);
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_BTA_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_BTA);
        break;
    ...
    int addFlags)
{
    ...
    bool const bOnBTA = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnBTA)
        {
            ...
            { // non-default path to BTA destination
                JLOG (j_.trace())
    ...
    {
        // add order books
        if (addFlags & afOB_BTA)
        {
            // to BTA only
            if (!bOnBTA && app_.getOrderBookDB ().isBookToBTA (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to BTA
                        ...
                        {
                            // destination is BTA, add account and path is
                            // complete
                            JLOG (j_.trace())
                        ...
                        book->getIssuerOut ()))
                    {
                            ...
                            (newPath[newPath.size() - 2].isOffer ()))
                        {
                            ...
                            newPath[newPath.size() - 1] = STPathElement (
                                ...
                                btaAccount(), book->getCurrencyOut(),
                                book->getIssuerOut());
                        }
                        else
                        {
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // bta book
                ret.push_back (Pathfinder::nt_BTA_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonBTA_to_BTA, {
            {1, "sxd"},       // gateway buys BTA
            {2, "saxd"},      // source -> gateway -> book(BTA) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-BTA to non-BTA (same currency)
    fillPaths(
        pt_nonBTA_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to BTA -> book ->
                            // destination
            {6, "saxfad"},
