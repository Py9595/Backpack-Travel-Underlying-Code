...
bool PathRequest::isValid (std::shared_ptr<RippleLineCache> const& crCache)
{
    ...
    if (! sleDest)
    {
        ...
        {
            // Only BTA can be send to a non-existent account.
    ...
    {
        bool const disallowBTA (
            sleDest->getFlags() & lsfDisallowBTA);

        auto usDestCurrID = accountDestCurrencies (
                *raDstAccount, crCache, ! disallowBTA);
    ...
    int const level, Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (! sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? btaAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = ! isBTA(issue.account)
            ? issue.account
