...
//   an exact match.
// - Real issuers must be specified for non-BTA.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        // book.  btaAccount() does double duty as signaling "this is an order
        // book".

        // Corresponds to "Implies an offer directory" in the diagram, currently
        // at http://goo.gl/Uj3HAB.

        auto type = isBTA(currency) ? STPathElement::typeCurrency
            : STPathElement::typeCurrency | STPathElement::typeIssuer;

        // The offer's output is what is now wanted.
        // btaAccount() is a placeholder for offers.
        resultCode = pushNode (type, btaAccount(), currency, issuer);
    }


    // For ripple, non-BTA, ensure the issuer is on at least one side of the
    // transaction.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    else if (hasIssuer && isBTA (node.issue_))
    {
        JLOG (j_.debug()) << "pushNode: issuer specified for BTA.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isBTA (node.issue_) ? btaAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isBTA(node.issue_.currency) ? btaAccount() : account);
    ...
    else
    {
        ...
        else if (isBTA (node.issue_.currency))
            node.issue_.account = btaAccount();
        else if (isBTA (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            resultCode   = pushImpliedNodes (
                btaAccount(), // Rippling, but offers don't have an account.
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isBTA(uMaxCurrencyID) ? btaAccount() : uSenderID;
    // Sender is always issuer for non-BTA.
    ...
    // BTA with issuer is malformed.
    if ((isBTA (uMaxCurrencyID) && !isBTA (uMaxIssuerID))
        || (isBTA (currencyOutID) && !isBTA (issuerOutID)))
    {
        JLOG (j_.debug())
            << "expandPath> issuer with BTA";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-BTA, issuer is always sending account.
    // - Trying to expand, not-compact.
    // - Every issuer will be traversed through.
    if (terStatus == tesSUCCESS)
    {
        terStatus   = pushNode (
            !isBTA(uMaxCurrencyID)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was BTA, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is BTA, offer next. Must go through issuer.
    ...
    if (terStatus == tesSUCCESS
        && !isBTA(currencyOutID)               // Next is not BTA
