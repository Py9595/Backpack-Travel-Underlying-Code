...
#endif
    explicit
    EitherAmount (BTAAmount const& a)
            :bta(a)
    {
#ifndef NDEBUG
        native = true;
...
inline
BTAAmount&
get<BTAAmount> (EitherAmount& amt)
{
    assert (amt.native);
    return amt.bta;
}
...
inline
BTAAmount const&
get<BTAAmount> (EitherAmount const& amt)
{
    assert (amt.native);
    return amt.bta;
}
...
toAmountSpec (STAmount const& amt)
{
    ...
    result.native = isBTA (amt);
    if (result.native)
    {
        result.bta = BTAAmount (sMant);
    }
    else
    {
