...
public:
        ...
        bool prevStepRedeems) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isBTA(id) || id == this->strandDst_)
                return parityRate;
    ...
    Callback& callback) const
{
    ...
    while (offers.step ())
    {
        ...
        // An account can always own BTA or their own IOUs.
        if (flowCross &&
            (!isBTA (offer.issueIn().currency)) &&
...
{
    bool const inBTA = isBTA (book.in.currency);
    bool const outBTA = isBTA (book.out.currency);
    if (inBTA && outBTA)
        return equalHelper<BTAAmount, BTAAmount,
            BookPaymentStep<BTAAmount, BTAAmount>> (step, book);
    if (inBTA && !outBTA)
        return equalHelper<BTAAmount, IOUAmount,
            BookPaymentStep<BTAAmount, IOUAmount>> (step, book);
    if (!inBTA && outBTA)
        return equalHelper<IOUAmount, BTAAmount,
            BookPaymentStep<IOUAmount, BTAAmount>> (step, book);
    if (!inBTA && !outBTA)
...
make_BookStepIX (
    ...
    Issue const& in)
{
    return make_BookStepHelper<IOUAmount, BTAAmount> (ctx, in, btaIssue());
}
...
make_BookStepXI (
    ...
    Issue const& out)
{
    return make_BookStepHelper<BTAAmount, IOUAmount> (ctx, btaIssue(), out);
}
