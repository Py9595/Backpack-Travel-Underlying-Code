...
bool checkNear (BTAAmount const& expected, BTAAmount const& actual)
{
    return expected == actual;
};

static
bool isBTAAccount (STPathElement const& pe)
{
    if (pe.getNodeType () != STPathElement::typeAccount)
        return false;
    return isBTA (pe.getAccountID ());
};
    ...
    Issue const& curIssue)
{
    ...
    if (ctx.isFirst && e1->isAccount () &&
        ...
        isBTA (e1->getCurrency ()))
    {
        return make_BTAEndpointStep (ctx, e1->getAccountID ());
    }

    if (ctx.isLast && isBTAAccount (*e1) && e2->isAccount())
        return make_BTAEndpointStep (ctx, e2->getAccountID());
    ...
    if (isBTA (curIssue.currency) && isBTA (outCurrency))
    {
        JLOG (j.warn()) << "Found bta/bta offer payment step";
        return {temBAD_PATH, std::unique_ptr<Step>{}};
    }

    assert (e2->isOffer ());

    if (isBTA (outCurrency))
        return make_BookStepIX (ctx, curIssue);

    if (isBTA (curIssue.currency))
    ...
    beast::Journal j)
{
    if (isBTA (src))
    {
        JLOG (j.debug()) << "toStrand with btaAccount as src";
        return {temBAD_PATH, Strand{}};
    }
    if (isBTA (dst))
    {
        JLOG (j.debug()) << "toStrand with btaAccount as dst";
        return {temBAD_PATH, Strand{}};
    }
    ...
    beast::Journal j)
{
    ...
    for (auto const& pe : path)
    {
        ...
        if (hasIssuer && isBTA(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};

        if (hasAccount && isBTA(pe.getAccountID()))
            return {temBAD_PATH, Strand{}};

        if (hasCurrency && hasIssuer &&
            isBTA(pe.getCurrency()) != isBTA(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};
    ...
    */
        ...
        */
        ...
        if (cur->isAccount() && next->isAccount())
        {
                ...
                curIssue.account != next->getAccountID ())
            {
                ...
                result.push_back (std::move (msr.second));
                impliedPE.emplace(STPathElement::typeAccount,
                    curIssue.account, btaCurrency(), btaAccount());
                cur = &*impliedPE;
            }
        }
        else if (cur->isAccount() && next->isOffer())
        ...
        else if (cur->isOffer() && next->isAccount())
        {
                ...
                !isBTA (next->getAccountID ()))
            {
                if (isBTA(curIssue))
                {
                    ...
                    {
                        // Last step. insert bta endpoint step
                        auto msr = make_BTAEndpointStep (ctx(), next->getAccountID());
                        if (msr.first != tesSUCCESS)
                            return {msr.first, Strand{}};
                        result.push_back(std::move(msr.second));
                    }
    ...
    auto checkStrand = [&]() -> bool {
        auto stepAccts = [](Step const& s) -> std::pair<AccountID, AccountID> {
            ...
            return std::make_pair(btaAccount(), btaAccount());
        };
...
bool
isDirectBtaToBta(Strand const& strand)
{
    return false;
}
...
template<>
bool
isDirectBtaToBta<BTAAmount, BTAAmount> (Strand const& strand)
{
...
template
bool
isDirectBtaToBta<BTAAmount, IOUAmount> (Strand const& strand);
template
bool
isDirectBtaToBta<IOUAmount, BTAAmount> (Strand const& strand);
template
bool
isDirectBtaToBta<IOUAmount, IOUAmount> (Strand const& strand);
