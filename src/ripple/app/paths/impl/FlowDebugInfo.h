...
struct FlowDebugInfo
{
    ...
    to_string (bool writePassInfo) const
    {
        ...
        if (writePassInfo)
        {
            ...
            auto writeNestedBtaAmtList = [&ostr, &writeBtaAmtList](
                std::vector<std::vector<EitherAmount>> const& amts) {
                ostr << '[';
                if (!amts.empty ())
                {
                    writeBtaAmtList(amts[0], '|');
                    for (size_t i = 1, e = amts.size (); i < e; ++i)
                    {
                        ostr << ';';
                        writeBtaAmtList(amts[i], '|');
                    }
                }
                ostr << ']';
            };
            ...
            if (passInfo.nativeIn)
                writeBtaAmtList (passInfo.in);
            else
                writeIouAmtList (passInfo.in);
            ostr << ", out_pass: ";
            if (passInfo.nativeOut)
                writeBtaAmtList (passInfo.out);
            else
                ...
                !passInfo.liquiditySrcIn.back ().empty ())
            {
                ostr << ", l_src_in: ";
                if (passInfo.nativeIn)
                    writeNestedBtaAmtList (passInfo.liquiditySrcIn);
                else
                    writeNestedIouAmtList (passInfo.liquiditySrcIn);
                ostr << ", l_src_out: ";
                if (passInfo.nativeOut)
                    writeNestedBtaAmtList (passInfo.liquiditySrcOut);
                else
...
{
    return {sb.balanceChanges (rv), sb.btaDestroyed ()};
}
...
balanceDiffsToString (boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& btaDestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", btaDestroyed: " << to_string (btaDestroyed);
    ostr << ", balanceDiffs: ";
