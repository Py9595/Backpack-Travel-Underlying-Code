...
// - If next node is an offer and output is non-BTA then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is BTA then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-BTA, current node is the issuer.
            JLOG (j_.trace())
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
        ...
        else
        {
            ...
            if (pathState_.inReq() >= zero)
            {
                ...
                // Limit BTA by available. No limit for non-BTA as issuer.
                if (isBTA (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            node().account_,
                            btaCurrency(),
                            btaAccount(),
                            fhIGNORE_FREEZE, viewJ)); // BTA can't be frozen
            ...
            else
            {
                ...
                // Deliver BTA to limbo.
                resultCode = accountSend(view(),
                    node().account_, btaAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())
