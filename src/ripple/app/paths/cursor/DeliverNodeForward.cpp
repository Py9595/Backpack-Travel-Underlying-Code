    ...
    bool callerHasLiquidity) const
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isBTA(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as BTA or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send BTA or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isBTA(node().issue_) ?
                        btaAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isBTA (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isBTA(previousNode().issue_.currency) ?
                        uInAccountID : btaAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
