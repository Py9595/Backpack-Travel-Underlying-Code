...
RippleCalc::Output RippleCalc::rippleCalculate (
    ...
    // Issuer:
    //      BTA: btaAccount()
    //  non-BTA: uSrcAccountID (for any issuer) or another account with
    //           trust node.
    STAmount const& saMaxAmountReq,             // --> -1 = no limit.

    // Issuer:
    //      BTA: btaAccount()
    //  non-BTA: uDstAccountID (for any issuer) or another account with
    //           trust node.
...
TER RippleCalc::rippleCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    ...
    // nodes.
    // XXX Might also make a BTA bridge by default.
