...
struct TxConsequences
{
    ...
    BTAAmount const potentialSpend;

    /// Constructor
    TxConsequences(ConsequenceCategory const category_,
        BTAAmount const fee_, BTAAmount const spend_)
        : category(category_)
        , fee(fee_)
...
/** Determine the BTA balance consequences if a transaction
    consumes the maximum BTA allowed.
