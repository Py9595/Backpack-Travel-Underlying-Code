...
#include <backpacktravel/app/storage/TableStorageItem.h>
...
preflight1 (PreflightContext const& ctx)
{
    ...
    if (!fee.native () || fee.negative () || !isLegalAmount (fee.bta ()))
    {
...
Transactor::checkFee (PreclaimContext const& ctx, std::uint64_t baseFee)
{
	...
	if (ctx.tx.isBackpacktravelAlliancesBaseType())
	{
		int btaDrops = 1000000;
		...
		auto extraAmount = new BTAAmount((int)(btaDrops * multiplier));
		feeDue += *extraAmount;
	}
    ...
    auto const sle = ctx.view.read(
        keylet::account(id));
    auto const balance = (*sle)[sfBalance].bta();
...
TER Transactor::payFee ()
{
    ...
    // VFALCO Should we call view().rawDestroyBTA() here as well?
...
TER Transactor::apply ()
{
    ...
    if (sle)
    {
        mPriorBalance   = STAmount ((*sle)[sfBalance]).bta ();
	...
	if (ctx_.tx.isBackpacktravelAlliancesBaseType())
	{
		checkAddChainIDSle();
...
void
Transactor::claimFee (BTAAmount& fee, TER terResult, std::vector<uint256> const& removedOffers)
{
    ctx_.discard();
    ...
    auto const balance = txnAcct->getFieldAmount (sfBalance).bta ();
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).bta ();
