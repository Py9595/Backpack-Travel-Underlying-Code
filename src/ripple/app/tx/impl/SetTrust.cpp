...
SetTrust::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.trace()) <<
            "Malformed transaction: specifies BTA as IOU";
...
SetTrust::doApply ()
{
    ...
    // user enough BTA to cover not only the account reserve
    // but the incremental reserve for the trust line as
    // well. A person with no intention of using the gateway
    // could use the extra BTA for their own purposes.

    BTAAmount const reserveCreate ((uOwnerCount < 2)
        ? BTAAmount (zero)
    ...
    if (sleRippleState)
    {
        ...
        else if (bReserveIncrease && mPriorBalance < reserveCreate)
        {
            ...
            // Another transaction could provide BTA to the account and then
