...
Payment::calculateMaxSpend(STTx const& tx)
{
    if (tx.isFieldPresent(sfSendMax))
    {
        ...
        return sendMax.native() ? sendMax.bta() : beast::zero;
    }
    /* If there's no sfSendMax in BTA, and the sfAmount isn't
    in BTA, then the transaction can not send BTA. */
    auto const& saDstAmount = tx.getFieldAmount(sfAmount);
    return saDstAmount.native() ? saDstAmount.bta() : beast::zero;
}
...
Payment::preflight (PreflightContext const& ctx)
{
    ...
    // isZero() is BTA.  FIX!
    bool const bBTADirect = uSrcCurrency.isZero () && uDstCurrency.isZero ();
    ...
    if (bBTADirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: " <<
            "SendMax specified for BTA to BTA.";
        return temBAD_SEND_BTA_MAX;
    }
    if (bBTADirect && bPaths)
    {
        // BTA is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Paths specified for BTA to BTA.";
        return temBAD_SEND_BTA_PATHS;
    }
    if (bBTADirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Partial payment specified for BTA to BTA.";
        return temBAD_SEND_BTA_PARTIAL;
    }
    if (bBTADirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Limit quality specified for BTA to BTA.";
        return temBAD_SEND_BTA_LIMIT;
    }
    if (bBTADirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "No ripple direct specified for BTA to BTA.";
        return temBAD_SEND_BTA_NO_DIRECT;
    }
...
Payment::doApply ()
{
    ...
    else
    {
        ...
        auto const mmm = std::max(reserve,
            ctx_.tx.getFieldAmount (sfFee).bta ());

        if (mPriorBalance < saDstAmount.bta () + mmm)
        {
            // Vote no. However the transaction might succeed, if applied in
            // a different order.
            JLOG(j_.trace()) << "Delay transaction: Insufficient funds: " <<
                " " << to_string (mPriorBalance) <<
                " / " << to_string (saDstAmount.bta () + mmm) <<
                " (" << to_string (reserve) << ")";
