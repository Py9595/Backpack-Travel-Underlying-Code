    ...
    , journal_ (journal)
{
    ...
    // flavor we expect:
    assert (cross_type != CrossType::BtaToIou ||
        (isBTA (issue_in ()) && !isBTA (issue_out ())));

    assert (cross_type != CrossType::IouToBta ||
        (!isBTA (issue_in ()) && isBTA (issue_out ())));

    // And make sure we're not crossing BTA for BTA
    assert (!isBTA (issue_in ()) || !isBTA (issue_out ()));
    ...
    Rate const& rate_out)
{
    ...
    // of an autobridge.
    if (cross_type_ == CrossType::BtaToIou && (remaining_.in < f.order.in))
    {
    ...
    Rate const& rate_in)
{
    ...
    if (!sell_ && cross_type_ == CrossType::IouToBta)
    {
...
BasicTaker::do_cross (Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    if (cross_type_ == CrossType::BtaToIou)
    {
        result = flow_bta_to_iou (offer, quality, owner_funds, taker_funds,
            out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToBta)
    {
        result = flow_iou_to_bta (offer, quality, owner_funds, taker_funds,
    ...
    Amounts offer2, Quality quality2, AccountID const& owner2)
{
    ...
    // The amount available to flow via BTA is the amount that the owner of the
    ...
    // of BTA that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out BTA of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // BTA to the largest of the two offers.
    auto bta_funds = get_funds (owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace()) <<
            "The bridge endpoints are owned by the same account.";
        bta_funds = std::max (offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      bta: " << format_amount (bta_funds);
    }
    ...
    auto flow1 = flow_iou_to_bta (offer1, quality1, bta_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check ())
        Throw<std::logic_error> ("Computed flow1 fails sanity check.");

    auto flow2 = flow_bta_to_iou (offer2, quality2, leg2_out_funds, bta_funds, leg2_rate);
    ...
    // equalize them, so that the amount of BTA that flows out of the first leg
    // is the same as the amount of BTA that flows into the second leg. We take
...
TER Taker::transferBTA (
    AccountID const& from,
    AccountID const& to,
    STAmount const& amount)
{
    if (!isBTA (amount))
        Throw<std::logic_error> ("Using transferBTA with IOU");
    ...
    return ripple::transferBTA (view_, from, to, amount, journal_);
}
...
{
    if (isBTA (amount))
        Throw<std::logic_error> ("Using redeemIOU with BTA");
...
{
    if (isBTA (amount))
        Throw<std::logic_error> ("Using issueIOU with BTA");
...
Taker::fill (BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    if (cross_type () != CrossType::BtaToIou)
    {
        assert (!isBTA (flow.order.in));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferBTA (account (), offer.owner (), flow.order.in);
    }

    // Now send funds from the account whose offer we're taking
    if (cross_type () != CrossType::IouToBta)
    {
        assert (!isBTA (flow.order.out));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferBTA (offer.owner (), account (), flow.order.out);
    }
    ...
    BasicTaker::Flow const& flow2, Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over BTA
    if (result == tesSUCCESS)
        result = transferBTA (leg1.owner (), leg2.owner (), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be BTA.
    if (isBTA (offer.amount ().in) && isBTA (offer.amount ().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, BTA must can't be the input to the first leg
    // or the output of the second leg.
    if (isBTA (leg1.amount ().in) || isBTA (leg2.amount ().out))
        return tefINTERNAL;
...
Taker::calculateRate (
    ApplyView const& view,
        AccountID const& issuer,
            AccountID const& account)
{
    return isBTA (issuer) || (account == issuer)
        ? parityRate
