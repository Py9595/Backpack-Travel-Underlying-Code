    ...
    std::shared_ptr <SLE const> const& after)
{
    if(before)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ -= (*before)[sfBalance].bta().drops();
            break;
        case ltPAYCHAN:
            drops_ -= ((*before)[sfAmount] - (*before)[sfBalance]).bta().drops();
            break;
        case ltESCROW:
            drops_ -= (*before)[sfAmount].bta().drops();
            break;
        default:
            break;
        }
    ...
    if(after)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ += (*after)[sfBalance].bta().drops();
            break;
        case ltPAYCHAN:
            if (! isDelete)
                drops_ += ((*after)[sfAmount] - (*after)[sfBalance]).bta().drops();
            break;
        case ltESCROW:
            if (! isDelete)
                drops_ += (*after)[sfAmount].bta().drops();
            break;
        default:
            break;
        }
...
bool
BTANotCreated::finalize(STTx const& tx, TER /*tec*/, beast::Journal const& j)
{
    auto fee = tx.getFieldAmount(sfFee).bta().drops();
    if(-1*fee <= drops_ && drops_ <= 0)
        return true;

    JLOG(j.fatal()) << "Invariant failed: BTA net change was " << drops_ <<
        " on a fee of " << fee;
    return false;
}
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& balance)
    {
        ...
        auto const drops = balance.bta().drops();
...
bool
BTABalanceChecks::finalize(STTx const&, TER, beast::Journal const& j)
{
    if (bad_)
    {
        JLOG(j.fatal()) << "Invariant failed: incorrect account BTA balance";
        return false;
    }
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& pays, STAmount const& gets)
    {
        ...
        // Can't have an BTA to BTA offer:
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& amount)
    {
        ...
        if (amount.bta().drops() <= 0)
            return true;

        if (amount.bta().drops() >= SYSTEM_CURRENCY_START)
            return true;
    ...
    std::shared_ptr <SLE const> const& after)
{
    if (after && after->getType() == ltRIPPLE_STATE)
    {
        ...
        btaTrustLine_ =
            after->getFieldAmount (sfLowLimit).issue() == btaIssue() ||
            after->getFieldAmount (sfHighLimit).issue() == btaIssue();
    }
}

bool
NoBTATrustLines::finalize(STTx const&, TER, beast::Journal const& j)
{
    if (! btaTrustLine_)
        return true;

    JLOG(j.fatal()) << "Invariant failed: an BTA trust line was created";
    return false;
}
