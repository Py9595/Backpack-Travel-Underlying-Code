...
/**
 * @brief Invariant: A transaction must not create BTA and should only destroy
 * BTA, up to the transaction fee.
 *
 ...
 * and payment channels.  Remove from the total any previous BTA values and add
 * to the total any new BTA values. The net balance of a payment channel is
...
/**
 * @brief Invariant: An account BTA balance must be in BTA and take a value
                     between 0 and SYSTEM_CURRENCY_START drops, inclusive.
 */
class BTABalanceChecks
{
    bool bad_ = false;
...
/**
 * @brief Invariant: Trust lines using BTA are not allowed.
 */
class NoBTATrustLines
{
    bool btaTrustLine_ = false;
