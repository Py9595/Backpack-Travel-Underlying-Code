...
SetAccount::preflight (PreflightContext const& ctx)
{
    ...
    // DisallowBTA
    //
    bool bSetDisallowBTA   = (uTxFlags & tfDisallowBTA) || (uSetFlag == asfDisallowBTA);
    bool bClearDisallowBTA = (uTxFlags & tfAllowBTA) || (uClearFlag == asfDisallowBTA);

    if (bSetDisallowBTA && bClearDisallowBTA)
    {
        JLOG(j.trace()) << "Malformed transaction: Contradictory flags set.";
        return temINVALID_FLAG;
    }
...
SetAccount::doApply ()
{
    ...
    // DisallowBTA
    //
    if (bSetDisallowBTA && !(uFlagsIn & lsfDisallowBTA))
    {
        JLOG(j_.trace()) << "Set lsfDisallowBTA.";
        uFlagsOut |= lsfDisallowBTA;
    }

    if (bClearDisallowBTA && (uFlagsIn & lsfDisallowBTA))
    {
        JLOG(j_.trace()) << "Clear lsfDisallowBTA.";
        uFlagsOut &= ~lsfDisallowBTA;
    }
