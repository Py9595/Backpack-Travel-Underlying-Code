...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) <<
            "Malformed offer: redundant (BTA for BTA)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code BTA.
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isBTA (takerAmount.in) && !isBTA (takerAmount.out));

    if (isBTA (takerAmount.in) || isBTA (takerAmount.out))
        Throw<std::logic_error> ("Bridging with BTA and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), btaIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (btaIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling BTA
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling BTA charged fees can
        ...
        // of the takerAmount currencies are BTA then we cross through an
        // additional path with BTA as the intermediate between two books.
...
CreateOffer::preCompute()
{
    ...
    if (pays_bta && !gets_bta)
        cross_type_ = CrossType::IouToBta;
    else if (gets_bta && !pays_bta)
        cross_type_ = CrossType::BtaToIou;
...
CreateOffer::applyGuts (Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        BTAAmount reserve = ctx_.view().fees().accountReserve(
