...
/*
    ...
    of BTA in its own ledger entry, until the escrow process
    ...
    sequestered BTA. If the escrow is, instead, canceled,
    the account which created the escrow will receive the
    sequestered BTA back instead.
...
EscrowCreate::doApply()
{
    ...
    {
        auto const balance = STAmount((*sle)[sfBalance]).bta();
        ...
        if (balance < reserve + STAmount(ctx_.tx[sfAmount]).bta())
            return tecUNFUNDED;
    }
