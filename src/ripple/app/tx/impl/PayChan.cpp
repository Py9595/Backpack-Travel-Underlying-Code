...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of BTA payments flowing
        in a single direction. A channel sequesters the owner's BTA in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of BTA to deposit in the channel immediately.
        SettleDelay
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of BTA delivered after this claim is processed (optional, not
            needed if just closing).
        Amount (optional)
            The amount of BTA the signature is for (not needed if equal to Balance or just
            closing the line).
        Signature (optional)
...
PayChanCreate::preflight (PreflightContext const& ctx)
{
    ...
    if (!isBTA (ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanCreate::preclaim(PreclaimContext const &ctx)
{
...

    {
        ...
        if ((*sled)[sfFlags] & lsfDisallowBTA)
            return tecNO_TARGET;
    }
...
PayChanClaim::preflight (PreflightContext const& ctx)
{
    ...
    if (bal && (!isBTA (*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isBTA (*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
    ...
    if (auto const sig = ctx.tx[~sfSignature])
    {
        ...
        auto const reqBalance = bal->bta ();
        auto const authAmt = amt ? amt->bta() : reqBalance;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount (sfBalance).bta ();
        auto const chanFunds = slep->getFieldAmount (sfAmount).bta ();
        auto const reqBalance = ctx_.tx[sfBalance].bta ();
        ...
        if (txAccount == src && ((*sled)[sfFlags] & lsfDisallowBTA))
            return tecNO_TARGET;

        (*slep)[sfBalance] = ctx_.tx[sfBalance];
        BTAAmount const reqDelta = reqBalance - chanBalance;
