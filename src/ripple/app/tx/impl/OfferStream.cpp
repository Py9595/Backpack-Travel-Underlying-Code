...
static
BTAAmount accountFundsHelper (ReadView const& view,
    AccountID const& id,
    BTAAmount const& amtDefault,
    Issue const& issue,
    FreezeHandling freezeHandling,
    beast::Journal j)
{
    return toAmount<BTAAmount> (
        accountHolds (view, id, issue.currency, issue.account, freezeHandling, j));
}
...
template class FlowOfferStream<BTAAmount, IOUAmount>;
template class FlowOfferStream<IOUAmount, BTAAmount>;

template class TOfferStreamBase<STAmount, STAmount>;
template class TOfferStreamBase<IOUAmount, IOUAmount>;
template class TOfferStreamBase<BTAAmount, IOUAmount>;
template class TOfferStreamBase<IOUAmount, BTAAmount>;
}
