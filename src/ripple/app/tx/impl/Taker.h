...
protected:
    ...
    struct Flow
    {
        ...
        {
            if (isBTA (order.in) && isBTA (order.out))
                return false;
...
private:
    ...
    Flow
    flow_bta_to_iou (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_out);

    Flow
    flow_iou_to_bta (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_in);
...
private:
    ...
    TER
    transferBTA (AccountID const& from, AccountID const& to, STAmount const& amount);
...
private:
    ...
    // The amount of BTA that flowed if we were autobridging
    STAmount bta_flow_;
