...
{
    bool toBTA = isBTA (book.out);
    std::lock_guard <std::recursive_mutex> sl (mLock);

    if (toBTA)
    {
        // We don't want to search through all the to-BTA or from-BTA order
        // books!
        for (auto ob: mSourceMap[book.in])
        {
            if (isBTA (ob->getCurrencyOut ())) // also to BTA
                return;
        }
    }
    else
...
bool OrderBookDB::isBookToBTA(Issue const& issue)
{
    std::lock_guard <std::recursive_mutex> sl (mLock);
    return mBTABooks.count(issue) > 0;
}
