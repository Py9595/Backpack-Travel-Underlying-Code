...
#include <backpacktravel/rpc/impl/TableAssistant.h>
...
public:
    ...
    */
	...
	void pubBackpacktravelAlliancesTxResult(const STTx& stTxn,
		const std::pair<std::string, std::string>& disposRes,bool validated);
	void pubBackpacktravelAlliancesTableTxs(const AccountID& ownerId, const std::string& sTableName, 
        ...
        bool bUnlimited, FailHard failType)
{
    ...
    if (stTx.isBackpacktravelAlliancesBaseType())
    {
    ...
    if (!human)
    {
        ...
        if (escalationMetrics)
        {
            ...
            /* Json::Value doesn't support uint64, so clamp to max
                ...
                probably isn't enough extant BTA to drive the factor
                that high.
            */
...
void NetworkOPsImp::PubValidatedTxForTable(const STTx& tx)
{
	...
	if (vecTxs.size() > 1)
	{
		...
		{
			pubBackpacktravelAlliancesTableTxs(item.first, item.second, tx, res);
		}
		if (listPair.size() > 0)
		{
			pubBackpacktravelAlliancesTxResult(tx, res, true);
		}
	}
	else
	{
	...
	const STTx& stTxn, const std::pair<std::string, std::string>& res,bool bValidated)
{
	...
	if (!bValidated && mSubTx.find(stTxn.getTransactionID()) != mSubTx.end())
	{
		...
		pubBackpacktravelAlliancesTxResult(stTxn, result, true);
	}

	pubBackpacktravelAlliancesTxResult(stTxn, res, bValidated);
	pubBackpacktravelAlliancesTableTxs(owner, sTableName, stTxn, res);
}

//publish results for chain-sql txs
void NetworkOPsImp::pubBackpacktravelAlliancesTxResult(const STTx& stTxn,
	...
	if (!subTx.empty())
	{
		...
		if (simiIt != subTx.end())
		{
			...
			if (p)
			{
				...
				p->send(jvObj, true);
				//for backpacktravelalliances type,subscribe db event
				if (stTxn.isBackpacktravelAlliancesBaseType() && bValidated)
				{
...
void NetworkOPsImp::pubBackpacktravelAlliancesTableTxs(const AccountID& ownerId, const std::string& sTableName, 
	const STTx& stTxn,const std::pair<std::string,std::string>& disposRes)
{
	ScopedLockType sl(mSubLock);
