              ...
              beast::Journal j)
{
    ...
    if (isBTA(currency))
    {
        return {btaLiquid (view, account, 0, j)};
    }
...
BTAAmount
btaLiquid (ReadView const& view, AccountID const& id,
    std::int32_t ownerCountAdj, beast::Journal j)
{
    ...
    if (fix1141 (view.info ().parentCloseTime))
    {
        ...
        auto const balance = view.balanceHook(id, btaAccount(), fullBalance);
    ...
    {
        // pre-switchover
        // BTA: return balance minus reserve
        ...
        return view.balanceHook(id, btaAccount(), amount).bta();
    }
}
    ...
    beast::Journal j)
{
    ...
    assert (!isBTA (uSenderID) && uSenderID != noAccount());
    assert (!isBTA (uReceiverID) && uReceiverID != noAccount());
    ...
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    ...
    assert (!isBTA (uSenderID) && !isBTA (uReceiverID));
    assert (uSenderID != uReceiverID);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    /* BTA send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            if (fv2Switch)
                view.creditHook (uSenderID, btaAccount (), saAmount, sndBal);

            // Decrement BTA balance.
    ...
    {
        // Increment BTA balance.
        auto const rcvBal = receiver->getFieldAmount (sfBalance);
        receiver->setFieldAmount (sfBalance, rcvBal + saAmount);

        if (fv2Switch)
            view.creditHook (btaAccount (), uReceiverID, saAmount, -rcvBal);
...
{
    assert (!isBTA (account) && !isBTA (issue.account));
