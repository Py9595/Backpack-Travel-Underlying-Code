...
struct CashSummary
{
    ...
    std::vector<std::pair<
        AccountID, BTAAmount>> btaChanges;
    ...
    void sort()
    {
        std::sort (btaChanges.begin(), btaChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            result.btaChanges.push_back (
                std::make_pair (prev[sfAccount], BTAAmount {0}));
            return true;
    ...
    else
    {
        ...
        case ltACCOUNT_ROOT:
        {
            auto const curBta = cur[sfBalance].bta();
            if (!before || (*before)[sfBalance].bta() != curBta)
                result.btaChanges.push_back (
                    std::make_pair (cur[sfAccount], curBta));
            return true;
        }
        case ltRIPPLE_STATE:
        {
...
{
private:
    // Note differences in destroyed BTA between two ApplyStateTables.
    struct DropsGone
    {
        BTAAmount lhs;
        BTAAmount rhs;
...
countKeys (detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn (countKeys(lhs.btaChanges,     rhs.btaChanges));
...
bool CashDiff::Impl::rmDust ()
{
    ...
    // amount differences: btaChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // btaChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust (lhsDiffs_.btaChanges, rhsDiffs_.btaChanges,
        [](BTAAmount const& lhs, BTAAmount const& rhs)
        {
        ...
        CashFilter rhsFilter, detail::ApplyStateTable const& rhs)
{
    ...
    // btaChanges:
    setDiff (lhsDiffs.btaChanges, rhsDiffs.btaChanges, lhsDiffs_.btaChanges);
    setDiff (rhsDiffs.btaChanges, lhsDiffs.btaChanges, rhsDiffs_.btaChanges);
...
bool diffIsDust (STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling BTA is different from IOU.
    if (v1.native())
    {
