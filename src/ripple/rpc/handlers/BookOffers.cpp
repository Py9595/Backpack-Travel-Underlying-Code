...
Json::Value doBookOffers (RPC::Context& context)
{
    ...
    {
        pay_issuer = btaAccount ();
    }

    if (isBTA (pay_currency) && ! isBTA (pay_issuer))
        return RPC::make_error (
            rpcSRC_ISR_MALFORMED, "Unneeded field 'taker_pays.issuer' for "
            "BTA currency specification.");
    ...
    {
        get_issuer = btaAccount ();
    }


    if (isBTA (get_currency) && ! isBTA (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
    ...
    if (!isBTA (get_currency) && isBTA (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            "Invalid field 'taker_gets.issuer', expected non-BTA issuer.");
